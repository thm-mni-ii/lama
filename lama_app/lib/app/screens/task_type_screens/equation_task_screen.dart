    ...
    if (task.random.isNotEmpty) {
      ...
      for (int i = 0; i < 7; i++) {
        answers.add((int.parse(task.random[1]) +
                _r.nextInt(
                    int.parse(task.random[2]) - int.parse(task.random[1])))
            .toString());
      }
      if (task.operator == 2) {
        fullEquation.add(task.random[0]);
      ...
      fullEquation.add("=");
      if (task.operator == 1) number3 = 0;
      fullEquation.add(
          _randomRes(task.random[0], number1, number2, number3).toString());
      if (task.operator == 1) {
        _missing1 = _r.nextInt(4);
        while (_missing1 == 3) _missing1 = _r.nextInt(4);
      } else {
          ...
          itemBuilder: (context, index) {
            return Draggable<String>(
              ...
              childWhenDragging: EquationItemWidget(
                  constraints, LamaColors.bluePrimary, wrongAnswers[index],
                  fontSize: 20),
              child: EquationItemWidget(
                  constraints, LamaColors.blueAccent, wrongAnswers[index],
                  fontSize: 20),
              feedback: Material(
