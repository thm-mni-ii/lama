...
class ArrowButton {
  ...
  ArrowButton(this.game, this._relativeSize, this._arrowDirection,
      this._position, this._relativeOffsetY, this._onTap) {
    // button paint
    _paintButton = Paint();
  ...
  Path getArrowPath(int dir, double startX) {
    ...
    if (dir.isEven) {
      arrowPath.moveTo(
          startX + (2 * absoluteSize / 3), startY + (absoluteSize / 2));
      arrowPath.lineTo(
          startX + (absoluteSize / 3), startY + (absoluteSize / 2));
    } else {
      // vertical line
      arrowPath.moveTo(
          startX + (absoluteSize / 2), startY + (absoluteSize / 3));
      arrowPath.lineTo(
          startX + (absoluteSize / 2), startY + (2 * absoluteSize / 3));
    }
  ...
  void render(Canvas c) {
    ...
    // draw shadow
    c.drawArc(_rectButton.translate(_shadowWidth, _shadowWidth), 0, 17, true,
        _paintShadow);
    // draw button
