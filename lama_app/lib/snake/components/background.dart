...
class Background {
  ...
  Paint _controlBarPaint = Paint()..color = Color(0xFF34C935);
  ...
  void resize() {
    ...
    if (_controlBarRelativeHeight != null) {
      _controlBarRect = Rect.fromLTWH(
        0,
        game.screenSize.height -
            (_controlBarRelativeHeight! * game.screenSize.height),
        game.screenSize.width,
        game.screenSize.height,
      );
    }
    ...
    _fieldTiles = List.generate(
        ...
        (index) => Position(
            index % this.game.maxFieldX as int, index ~/ this.game.maxFieldX));

    // calculates the offset of the field depending on the [fieldOffsetY] of the SnakeGame
    _offsetX = game.screenSize.width > game.screenSize.height
        ? game.tileSize * this.game.fieldOffsetY
        : 0;
    _offsetY = game.screenSize.height > game.screenSize.width
        ? game.tileSize * this.game.fieldOffsetY
        : 0;
  ...
  void render(Canvas c) {
    ...
    if (_backgroundRect != null && _backgroundPaint != null) {
      c.drawRect(_backgroundRect!, _backgroundPaint!);
    }
    if (_fieldRect != null &&
        _backgroundPaint != null &&
        _fieldBorderPaint != null) {
    ...
    if (_chessOptic) {
      ...
      if (_fieldTiles != null) {
        for (var i = 0; i < _fieldTiles!.length; i++) {
          // tile rectangle
          if(_fieldTiles!=null&&_offsetX!=null &&_offsetY!=null){  var rect = Rect.fromLTWH(
