...
class PauseButton {
  ...
  /// [Paint] of the button
  final Paint _paintButton = Paint()..color = Color(0xffFBFEF5);
  ...
  PauseButton(this.game, this._relativeSize, this._position,
      this._relativeOffsetY, this._onTap) {
    resize();
  }
  ...
  void resize() {
    ...
    var spacePos = (this.game.screenSize.width -
            ((this.game.screenSize.width * _relativeOffsetX) * 2)) /
        5;
    ...
    _rectButton = Rect.fromLTWH(
        ...
        this._relativeOffsetY * game.screenSize.height +
            ((this.game.screenSize.width * this._relativeSize) / 2),
    ...
    _paintShadow.shader = LinearGradient(
      ...
      colors: <Color>[Color(0x72000000), Color(0x0)],
    ).createShader(Rect.fromLTWH(_rectButton.left, _rectButton.top,
        _rectButton.width, _rectButton.height));
  }
  ...
  Path getPausePath() {
    ...
    var startY = this._relativeOffsetY * game.screenSize.height +
        ((this.game.screenSize.width * this._relativeSize) / 2);

    returnPath.moveTo(
        _startX + 0.85 * (absoluteSize / 2), startY + (absoluteSize / 3));
    returnPath.lineTo(
        _startX + 0.85 * (absoluteSize / 2), startY + (2 * absoluteSize / 3));

    returnPath.moveTo(
        _startX + 1.15 * (absoluteSize / 2), startY + (absoluteSize / 3));
    returnPath.lineTo(
        _startX + 1.15 * (absoluteSize / 2), startY + (2 * absoluteSize / 3));
  ...
  Path getPlayPath() {
    ...
    returnPath.moveTo(
        _startX + 0.80 * (absoluteSize / 2), startY + (2 * absoluteSize / 3));
    returnPath.lineTo(
        _startX + 1.2 * (absoluteSize / 2), startY + (absoluteSize / 2));

    returnPath.moveTo(
        _startX + 0.80 * (absoluteSize / 2), startY + (absoluteSize / 3));
    returnPath.lineTo(
        _startX + 1.2 * (absoluteSize / 2), startY + (absoluteSize / 2));
